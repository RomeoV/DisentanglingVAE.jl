...
import Flux
import FluxTraining
import FluxTraining: HyperParameter, PropDict
...
@option mutable struct VAELoss{T}
  const reconstruction_loss::Function = logitbinarycrossentropy  # "Bernoulli" loss
...
function (loss::VAELoss)(ŷs::Tuple, ys::Tuple, model, _phase::FluxTraining.AbstractTrainingPhase)
  (x̄_lhs, μ̂_lhs, logσ̂²_lhs, x̄_rhs, μ̂_rhs, logσ̂²_rhs) = ŷs
  (_x_lhs, v_lhs, _x_rhs, v_rhs, ks_c) = ys
  escape_layer = model.encoder.layers[end].layers[end]
  ...
  return (
    ...
    # + loss.λ_l2_decoder * reg_l2(Flux.params(model.decoder))
    # covariance regularization
    + loss.λ_covariance * (cov_loss(μ̂_lhs) + cov_loss(μ̂_rhs))
    # directionality loss
    + loss.λ_directionality * directionality_loss(μ̂_lhs, μ̂_rhs)
    # direct supervision
    + loss.λ_direct_supervision * ( Flux.mse(μ̂_lhs, v_lhs)
                                  + Flux.mse(μ̂_rhs, v_rhs) )
  )
end
